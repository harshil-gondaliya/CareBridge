...
function DoctorProfile() {
  ...
  useEffect(() => {
    const loadDoctor = async () => {
      try {
        ...
        setError(apiError.response?.data?.message || 'Unable to load doctor profile.')
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    const loadAvailability = async () => {
      ...
      try {
        ...
        setError(apiError.response?.data?.message || 'Unable to load available slots.')
      } finally {
  ...
  const handleBookAppointment = async (event) => {
    ...
    try {
      ...
      setError(apiError.response?.data?.message || 'Unable to book this appointment.')
    } finally {
      setBooking(false)
    }
  }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[linear-gradient(180deg,_#eff6ff_0%,_#f0fdf4_100%)] px-6">
        <div className="max-w-xl rounded-[2rem] border border-rose-200 bg-white p-8 text-center shadow-xl shadow-rose-100/60">
          ...
          <p className="mt-4 text-base leading-7 text-slate-600">{error || 'This profile may no longer exist.'}</p>
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(14,165,233,0.16),_transparent_28%),radial-gradient(circle_at_bottom_right,_rgba(34,197,94,0.14),_transparent_25%),linear-gradient(180deg,_#f8fafc_0%,_#eff6ff_55%,_#f0fdf4_100%)] px-6 py-10">
      <div className="mx-auto max-w-6xl">
        <Link to="/doctors" className="inline-flex text-sm font-semibold text-sky-700 transition hover:text-sky-800">
          Back to doctors
        </Link>

        <section className="mt-6 overflow-hidden rounded-[2rem] border border-white/70 bg-white/90 shadow-2xl shadow-sky-100/70 backdrop-blur">
          <div className="grid lg:grid-cols-[0.9fr_1.1fr]">
            <div className="bg-[linear-gradient(180deg,_#0f172a,_#0f766e)] p-8 text-white">
              <div className="overflow-hidden rounded-[1.8rem] border border-white/10 bg-white/5">
                {doctor.profilePhoto ? (
                  <img src={doctor.profilePhoto} alt={doctor.name} className="h-[26rem] w-full object-cover" />
                ) : (
                  <div className="flex h-[26rem] items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.35),_transparent_38%),linear-gradient(135deg,_rgba(2,132,199,0.2),_rgba(22,163,74,0.2))] text-8xl font-bold text-white/80">
              ...
              <div className="mt-6 rounded-[1.5rem] border border-white/10 bg-white/5 p-5">
                ...
                <p className="mt-3 text-lg font-semibold">{doctor.email}</p>
              </div>
            </div>
            ...
            <div className="p-8 sm:p-10">
              ...
              <h1 className="mt-5 text-4xl font-bold tracking-tight text-slate-950">{doctor.name}</h1>
              <p className="mt-3 text-lg font-semibold text-sky-700">{doctor.specialization}</p>

              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                  <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Experience</p>
                  <p className="mt-3 text-xl font-bold text-slate-950">{doctor.experience} years</p>
                </div>
                <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                  <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Qualification</p>
                  <p className="mt-3 text-xl font-bold text-slate-950">{doctor.qualification || 'Not added yet'}</p>
                </div>
              </div>
              ...
              <div className="mt-8 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-6">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-3 text-sm leading-7 text-slate-600">
                    Select a date, pick a free slot from the doctor&apos;s published schedule,
                    and send a short note describing the reason for your visit.
                  </p>
                </div>
