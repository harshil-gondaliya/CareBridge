...
function Timeline() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.14),_transparent_24%),linear-gradient(180deg,_#f8fafc_0%,_#f0fdfa_45%,_#ecfeff_100%)] px-6 py-10">
      <section className="mx-auto max-w-5xl rounded-[2rem] border border-white/70 bg-white/90 p-8 shadow-2xl shadow-sky-100/60">
        <div className="max-w-2xl">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-sky-700">Medical Timeline</p>
          <h1 className="mt-4 text-4xl font-bold tracking-tight text-slate-950">Your complete healthcare history</h1>
              ...
              {timeline.map((item) => {
                ...
                return (
                  <article key={`${item.type}-${item.data._id}`} className="relative">
                    <div className={`absolute -left-[1.95rem] top-6 flex h-6 w-6 items-center justify-center rounded-full text-xs shadow-sm ${theme.badge}`}>
                      {theme.icon}
                    </div>

                    <div className={`rounded-[1.5rem] border p-6 shadow-sm ${theme.card}`}>
                      ...
                      {item.type === 'report' ? (
                        <div className="mt-4 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                          <a href={item.data.imageUrl} target="_blank" rel="noreferrer" className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                            <img src={item.data.imageUrl} alt="Uploaded report" className="h-48 w-full object-cover" />
                          </a>
